/**
 * ClawHub Installer - Handles environment setup for Termux
 */
...
export function installClawHub() {
  console.log('Installing ClawHub...');

  try {
    execSync('npm install -g clawhub', { stdio: 'inherit' });
    return true;
  } catch (err) {
    console.error('Failed to install ClawHub:', err.message);
    console.log('You may need to install it manually: npm install -g clawhub');
    return false;
  }
}
...
export function configureTermux() {
  ...
  const wakeLockScript = path.join(HOME, '.clawhub', 'wakelock.sh');
  const wakeLockContent = `#!/bin/bash
# Keep Termux awake while ClawHub runs
termux-wake-lock
...
export function getInstallStatus() {
  ...
  // Check if clawhub exists in proot ubuntu
  // Primary: fast filesystem check (avoids proot exec which can timeout on first login)
  let hasClawHubInProot = false;
  if (hasUbuntu) {
    try {
      const clawhubPkg = path.join(PROOT_ROOTFS, 'ubuntu', 'usr', 'local', 'lib', 'node_modules', 'clawhub', 'package.json');
      const hasNode = fs.existsSync(path.join(PROOT_ROOTFS, 'ubuntu', 'usr', 'local', 'bin', 'node'));
      hasClawHubInProot = fs.existsSync(clawhubPkg) && hasNode;
    } catch { /* check failed */ }

    // Fallback: proot exec with longer timeout and login shell
    if (!hasClawHubInProot) {
      try {
        execSync('proot-distro login ubuntu -- bash -lc "command -v clawhub"', { stdio: 'pipe', timeout: 30000 });
        hasClawHubInProot = true;
      } catch { /* not installed */ }
    }
  }
  ...
  try {
    const prootBypassPath = path.join(PROOT_ROOTFS, 'ubuntu', 'root', '.clawhub', 'bionic-bypass.js');
  ...
  return {
    proot: hasProot,
    ubuntu: hasUbuntu,
    openClawInProot: hasClawHubInProot,
    ...
    openClaw: (() => {
      try {
        execSync('command -v clawhub', { stdio: 'pipe' });
        return true;
      } catch { return false; }
    })()
  };
...
export function setupProotUbuntu() {
  console.log('Setting up Node.js and ClawHub in Ubuntu...');
...
`;
  ...
  const prootBypassPath = path.join(PROOT_UBUNTU_ROOT, '.clawhub', 'bionic-bypass.js');
  ...
  try {
    ...
    const exportLine = 'export NODE_OPTIONS="--require /root/.clawhub/bionic-bypass.js"';
...
export function runInProot(command) {
  const nodeOptions = '--require /root/.clawhub/bionic-bypass.js';
