/**
 * ClawHub-Termux - Main entry point
 */
...
function printHelp() {
  console.log(`
Usage: clawhubx <command> [args...]

Commands:
  setup       Full installation (proot + Ubuntu + ClawHub)
  status      Check installation status
  start       Start ClawHub gateway (inside proot)
  shell       Open Ubuntu shell with ClawHub ready
  help        Show this help message

  Any other command is passed directly to clawhub in proot:
    clawhubx onboarding      → clawhub onboarding
    clawhubx gateway -v      → clawhub gateway -v
    clawhubx doctor          → clawhub doctor
    clawhubx <anything>      → clawhub <anything>

Examples:
  clawhubx setup             # First-time setup
  clawhubx start             # Start gateway
  clawhubx onboarding        # Configure API keys
  clawhubx shell             # Enter Ubuntu shell
`);
}

async function runSetup() {
  console.log('Starting ClawHub setup for Termux...\n');
  console.log('This will install: proot-distro → Ubuntu → Node.js 22 → ClawHub\n');
  ...
  // Step 3: Setup Node.js and ClawHub in Ubuntu
  console.log('[3/5] Setting up Node.js and ClawHub in Ubuntu...');
  status = getInstallStatus();
  if (!status.openClawInProot) {
    setupProotUbuntu();
  } else {
    console.log('  ✓ ClawHub already installed in proot');
  }
  console.log('');
  ...
  console.log('  1. Run onboarding: clawhubx onboarding');
  console.log('     → Select "Loopback (127.0.0.1)" when asked!');
  console.log('  2. Start gateway:  clawhubx start');
  console.log('');
...
function showStatus() {
  ...
  if (status.ubuntu) {
    ...
    console.log(`  ClawHub:         ${status.openClawInProot ? '✓ installed' : '✗ not installed'}`);
  ...
  if (status.proot && status.ubuntu && status.openClawInProot) {
    ...
    console.log('  clawhubx start       # Start gateway');
    console.log('  clawhubx onboarding  # Configure API keys');
    console.log('  clawhubx shell       # Enter Ubuntu shell');
  } else {
    console.log('Status: ✗ Setup incomplete');
    console.log('Run: clawhubx setup');
  }
}
...
function startGateway() {
  ...
  if (!status.proot || !status.ubuntu) {
    console.error('proot/Ubuntu not installed. Run: clawhubx setup');
    process.exit(1);
  }

  if (!status.openClawInProot) {
    console.error('ClawHub not installed in proot. Run: clawhubx setup');
    process.exit(1);
  }
  ...
  const spinner = setInterval(() => {
    if (!started) {
      process.stdout.write(`\r${frames[i++ % frames.length]} Starting ClawHub gateway...`);
    }
  }, 80);
  ...
  const checkDashboard = setInterval(async () => {
    ...
    try {
      ...
      if (response.ok || response.status < 500) {
        ...
        console.log('✓ ClawHub gateway started!\n');
  ...
  const gateway = runInProot('clawhub gateway --verbose');
  ...
  gateway.on('close', (code) => {
    ...
    clearInterval(checkDashboard);
    if (!started) {
      console.log('\nGateway exited before starting. Run: clawhubx onboarding');
    }
...
function runOpenclawCommand(args) {
  ...
  if (!status.proot || !status.ubuntu || !status.openClawInProot) {
    console.error('Setup not complete. Run: clawhubx setup');
    process.exit(1);
  }

  const command = args.join(' ');
  console.log(`Running: clawhub ${command}\n`);
  ...
  const proc = runInProot(`clawhub ${command}`);
...
export async function main(args) {
  ...
  switch (command) {
    ...
    default:
      // Pass any other command to clawhub in proot
