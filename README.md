...
[![Download APK](https://img.shields.io/badge/Download-APK-green?style=for-the-badge&logo=android)](https://github.com/mithun50/clawhub-termux/releases/latest)
[![Build Flutter APK & AAB](https://github.com/mithun50/clawhub-termux/actions/workflows/flutter-build.yml/badge.svg)](https://github.com/mithun50/clawhub-termux/actions/workflows/flutter-build.yml)
[![npm version](https://img.shields.io/npm/v/clawhub-termux?color=blue&label=npm)](https://www.npmjs.com/package/clawhub-termux)
...
[![PRs Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg)](https://github.com/mithun50/clawhub-termux/pulls)

<p align="center">
  <img src="assets/ic_launcher.png" alt="ClawHub App Mockup" width="700"/>
</p>

> Run **ClawHub AI Gateway** on Android — standalone Flutter app with built-in terminal, web dashboard, optional dev tools, and one-tap setup. Also available as a Termux CLI package.
...
ClawHub brings the [ClawHub](https://github.com/anthropics/clawhub) AI gateway to Android. It sets up a full Ubuntu environment via proot, installs Node.js and ClawHub, and provides a native Flutter UI to manage everything — no root required.

### Two Ways to Use

| | **Flutter App** (Standalone) | **Termux CLI** |
|---|---|---|
| Install | Build APK or download release | `npm install -g clawhub-termux` |
| Setup | Tap "Begin Setup" | `clawhubx setup` |
| Gateway | Tap "Start Gateway" | `clawhubx start` |
...
### Flutter App
- **One-Tap Setup** — Downloads Ubuntu rootfs, Node.js 22, and ClawHub automatically
...
- **Configure Menu** — Run `clawhub configure` in a built-in terminal to manage gateway settings
...
The gateway's `clawhub.json` is automatically patched before startup to clear `denyCommands` and set `allowCommands` for all 15 commands.

### Termux CLI
- **One-Command Setup** — Installs proot-distro, Ubuntu, Node.js 22, and ClawHub
...
1. Download the latest APK from [Releases](https://github.com/mithun50/clawhub-termux/releases)
...
```bash
git clone https://github.com/mithun50/clawhub-termux.git
cd clawhub-termux/flutter_app
...
```bash
curl -fsSL https://raw.githubusercontent.com/mithun50/clawhub-termux/main/install.sh | bash
```

#### Or via npm

```bash
npm install -g clawhub-termux
clawhubx setup
```
...
| **Storage** | ~500MB for Ubuntu + Node.js + ClawHub |
...
```bash
# First-time setup (installs proot + Ubuntu + Node.js + ClawHub)
clawhubx setup

# Check installation status
clawhubx status

# Start ClawHub gateway
clawhubx start
...
# Any ClawHub command works directly
clawhubx doctor
clawhubx gateway --verbose
```
...
│   ├── configure_screen.dart       # clawhub configure terminal
│   ├── web_dashboard_screen.dart   # WebView for ClawHub dashboard
...
When running onboarding (in-app or via `clawhubx onboarding`):
...
Versions before v1.8.4 automatically requested full storage access (`MANAGE_EXTERNAL_STORAGE`) on launch. Combined with symlinks inside the proot rootfs pointing to `/sdcard`, cleanup operations could follow those symlinks and delete real user files. **This has been fixed** — storage permission is no longer auto-requested, symlinks are not followed during deletion, and a path boundary check prevents any deletion outside the app's private directory. If you were affected, see [#67](https://github.com/mithun50/clawhub-termux/issues/67).

To revoke storage permission: Android Settings > Apps > ClawHub > Permissions > Files and media > Don't allow.
...
```bash
mkdir -p ~/.clawhub
cat > ~/.clawhub/bionic-bypass.js << 'EOF'
const os = require('os');
...
```bash
echo 'export NODE_OPTIONS="--require ~/.clawhub/bionic-bypass.js"' >> ~/.bashrc
source ~/.bashrc
```
...
```bash
clawhub onboarding  # Select "Loopback (127.0.0.1)"
clawhub gateway --verbose
```
...
**Imran | [ArchClaw]](https://play.google.com/store/apps/dev?id=8262374975871504599)
...
- Play Store: [ArchClaw Apps](https://play.google.com/store/apps/dev?id=8262374975871504599)
- Contact: [archclawextra@gmail.com](mailto:nxgextra@gmail.com)
...
[![Star History Chart](https://api.star-history.com/svg?repos=mithun50/clawhub-termux&type=Date)](https://star-history.com/#mithun50/clawhub-termux&Date)


<p align="center">
  Made with &#10084;&#65039; for the Android community by <a href="https://github.com/mithun50">Imran | [ArchClaw]</b>
</p>
