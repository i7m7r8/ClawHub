...
class NodeService {
  ...
  /// 3. clawhub.json config file (source of truth — fallback when URL is stale or missing)
  Future<String?> _readGatewayToken() async {
    ...
    // 3. Read directly from clawhub.json — the source of truth (#94).
    ...
    try {
      ...
      if (raw != null) {
        ...
        if (token is String && token.isNotEmpty) {
          _log('[NODE] Gateway token read from clawhub.json config');
          return token;
        }
      }
    } catch (e) {
      _log('[NODE] Could not read token from clawhub.json: $e');
    }
    ...
    try {
      ...
      if (code != null) {
        ...
        if (isLocal) {
          ...
          try {
            await NativeBridge.runInProot('clawhub nodes approve $code');
