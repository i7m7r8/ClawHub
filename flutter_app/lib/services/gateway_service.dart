...
class GatewayService {
  ...
  Future<void> _writeNodeAllowConfig() async {
    ...
    // Use a Node.js one-liner to safely merge into existing clawhub.json
...
''';
  ...
  /// Repair clawhub.json on disk — fixes corrupted model entries and ensures
  ...
  /// Read the actual gateway auth token from clawhub.json config file (#74, #82).
  ...
  Future<void> _checkHealth() async {
    try {
      ...
      if (response.statusCode < 500 && _state.status != GatewayStatus.running) {
        // Read the actual token from clawhub.json — source of truth (#74, #82).
